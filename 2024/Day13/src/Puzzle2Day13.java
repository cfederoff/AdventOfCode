import java.io.BufferedReader;
import java.io.FileReader;

public class Puzzle2Day13 {
    public static void main(String[] args){
        try {
            BufferedReader file = new BufferedReader(new FileReader("2024/Day13/InputDay13.txt"));
            String line;
            long[][] matrix = {{0,0},{0,0}};
            long[] solution = {0,0};
            long total = 0;
            double a = Math.pow(10,13);
            long addition = (long) a;
            while ((line = file.readLine()) != null){
                matrix[0][0] = Integer.parseInt(line.split(" |: |, ")[2].substring(2));
                matrix[1][0] = Integer.parseInt(line.split(" |: |, ")[3].substring(2));
                line = file.readLine();
                matrix[0][1] = Integer.parseInt(line.split(" |: |, ")[2].substring(2));
                matrix[1][1] = Integer.parseInt(line.split(" |: |, ")[3].substring(2));
                line = file.readLine();
                solution[0] = Integer.parseInt(line.split(" |: |, ")[1].substring(2));
                solution[1] = Integer.parseInt(line.split(" |: |, ")[2].substring(2));
                line = file.readLine();
                long detA =  matrix[0][0] *matrix[1][1] - matrix[0][1] *matrix[1][0];
                long yArea = matrix[0][0] * (solution[1]+addition) - ((solution[0]+addition) * matrix[1][0]);
                long xArea = (solution[0]+addition) *matrix[1][1] - (solution[1]+addition) * matrix[0][1];
                double xfloat = (double) xArea/detA;
                double yfloat = (double) yArea/detA;
                if ((((long) xfloat) == xfloat) && ((long) xfloat) == xfloat){
                    long x = (long) xfloat;
                    long y = (long) yfloat;
                    total += 3 * x + y;
                }
            }
            System.out.println(total);
        } catch (Exception e){
            System.out.println(e);
        }
    }
}
